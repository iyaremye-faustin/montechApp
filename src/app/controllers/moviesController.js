...
import { newMovie,getMovies,getAMovie,deleteMovie } from "../../core/services/movieService";
...
export const viewCategories=async(req,res)=>{
  try {
    ...
    return handleErrorResponse(res, 500, "couldn't fetch categories", [],false);
  }
}
...
export const geMovieById=async(req,res)=>{
  ...
  if (error) return handleErrorResponse(res, 500, "couldn't fetch movie", error,false);
  if (movie === null){
...
export const updateMovie=async(req,res)=>{
  ...
  if (movie === null) return handleErrorResponse(res, 404, "movie not found", [],false);
  if (req.body.title) {
...
export const removeMovie=async(req,res)=>{
  ...
  if(error){
    return handleErrorResponse(res, 500, "Unable to remove a movie", error,false);
  }
  else{
